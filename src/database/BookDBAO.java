...
public class BookDBAO {
    ...
    public static String url = "jdbc:mongodb://127.0.0.1/test";
  ...
  //  private MongoDatabase db;
   // private MongoClient mongoClient;
    ...
    public BookDBAO(MongoClient mongo) throws Exception {
        try {
        	this.col = mongo.getDB("test").getCollection("books");
    ...
    public List<BookDetails> readAllBooks() {
		List<BookDetails> data = new ArrayList<BookDetails>();
		DBCursor cursor = col.find();
		while (cursor.hasNext()) {
			DBObject doc = cursor.next();
			BookDetails p = BooksMapper.toBook(doc);
			data.add(p);
		}
		return data;
	}
    
    public BookDetails getBookDetails(String p) {
		DBObject query = BasicDBObjectBuilder.start()
				.append("id", p).get();
		DBObject data = this.col.findOne(query);
		return BooksMapper.toBook(data);
	}
